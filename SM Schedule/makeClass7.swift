...
class makeClass7: UIViewController {
    ...
    let classTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Yellow Class"
    ...
    func handleNext(){
        ...
        let class7 = classTextField.text
        let class7Location = locationTextField.text
        if class7 != nil && class7Location != nil {
            ref.child("classes").child("class7").setValue(class7)
            ref.child("locations").child("class7").setValue(class7Location)
            presentViewController(makeClass8(), animated: true, completion: nil)
        }else {
            print("invalid class")
        }
