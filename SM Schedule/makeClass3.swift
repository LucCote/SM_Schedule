...
class makeClass3: UIViewController {
    ...
    let classTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Red Class"
    ...
    func handleNext(){
        ...
        let class3 = classTextField.text
        let class3Location = locationTextField.text
        if class3 != nil && class3Location != nil {
            ref.child("classes").child("class3").setValue(class3)
            ref.child("locations").child("class3").setValue(class3Location)
            presentViewController(makeClass4(), animated: true, completion: nil)
        }else {
            print("invalid class")
        }
