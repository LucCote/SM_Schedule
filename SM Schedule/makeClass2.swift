...
class makeClass2: UIViewController {
    ...
    let classTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Plum Class"
    ...
    func handleNext(){
        ...
        let class2 = classTextField.text
        let class2Location = locationTextField.text
        if class2 != nil && class2Location != nil {
            ref.child("classes").child("class2").setValue(class2)
            ref.child("locations").child("class2").setValue(class2Location)
            presentViewController(makeClass3(), animated: true, completion: nil)
        }else {
            print("invalid class")
        }
