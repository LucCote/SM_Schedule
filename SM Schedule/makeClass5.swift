...
class makeClass5: UIViewController {
    ...
    let classTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Blue Class"
    ...
    func handleNext(){
        ...
        let class5 = classTextField.text
        let class5Location = locationTextField.text
        if class5 != nil && class5Location != nil {
            ref.child("classes").child("class5").setValue(class5)
            ref.child("locations").child("class5").setValue(class5Location)
            presentViewController(makeClass6(), animated: true, completion: nil)
        }else {
            print("invalid class")
        }
