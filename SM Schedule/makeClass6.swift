...
class makeClass6: UIViewController {
    ...
    let classTextField: UITextField = {
        let tf = UITextField()
        tf.placeholder = "Enter Brown Class"
    ...
    func handleNext(){
        ...
        let class6 = classTextField.text
        let class6Location = locationTextField.text
        if class6 != nil && class6Location != nil {
            ref.child("classes").child("class6").setValue(class6)
            ref.child("locations").child("class6").setValue(class6Location)
            presentViewController(makeClass7(), animated: true, completion: nil)
        }else {
            print("invalid class")
        }
